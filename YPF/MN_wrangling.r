...
source("C:\\Users\\SE31886\\Documents\\Prediccion_MN\\PROP_ORGANICO\\script\\parser_TODO_VACA_MUERTA.R")
...
#imputo variable AT90. Para todo valor >200, reemplazo por 200 (PEDIDO DE ALBERTO)
#datos_POZOS_DET_temp$AT90 <- sapply(datos_POZOS_DET_temp$AT90, function(x) ifelse(x>200, 200, x))

#imputacion borrando linea entera
datos_POZOS_DET_temp  <- datos_POZOS_DET_temp  %>% filter(AT90<=200)

#Agregar variable RO como constante, segun los valores de cocina
...
#Agregar transformacion decreciente de RO en funcion de DEPTH
#Desde el tope de cocina y hacia arriba (menor DEPTH), el RO decrece -0.1 por cada 200 metros de DEPTH.
...
#funcion de transformacion lineal. CUIDADO! reemplaza los valores de COCINA tmb que deben ser constantes!
...
#la funcion transformadora lineal reemplaza los valores de COCINA, que en realidad deben ser constantes. Para corregir, utilizo el flag "COCINA":
datos_POZOS_DET <- datos_POZOS_DET %>% mutate(RO=case_when(
  WELL == "LCAV-21"            & COCINA == 1  ~ (1.3),
  WELL == "LRI-6(H)"           & COCINA == 1  ~ (1.6),
  WELL == "YPF.ADLA-1001(H)ST" & COCINA == 1  ~ (1.55),
  WELL == "YPF.NQ.EOR-31"      & COCINA == 1  ~ (1.45),
  WELL == "YPF.NQ.EOR-35D"     & COCINA == 1  ~ (1.55),
  WELL == "YPF.NQ.EOR-45"      & COCINA == 1  ~ (1.6),
  WELL == "YPF.NQ.LACH-14"     & COCINA == 1  ~ (0.9),
  WELL == "YPF.NQ.LACH-16"     & COCINA == 1  ~ (1),
  WELL == "YPF.NQ.LACH-17"     & COCINA == 1  ~ (1),
  WELL == "YPF.NQ.LACH-24"     & COCINA == 1  ~ (1.1),
  WELL == "YPF.NQ.LCAV-24"     & COCINA == 1  ~ (1.22),
  WELL == "YPF.NQ.LLL-993"     & COCINA == 1  ~ (1.08),
  WELL == "YPF.NQ.LLL-997"     & COCINA == 1  ~ (1.2),
  WELL == "YPF.NQ.RDM-184H"    & COCINA == 1  ~ (1.78),
...
#datos_POZOS_DET$AT90<-signif(datos_POZOS_DET_temp$AT90,4)
#datos_POZOS_DET$MN_XO_AD<-signif(datos_POZOS_DET_temp$MN_XO_AD,4)
...
#Agrupacion por metro. df total, solo cocina y resto de niveles
...
#Redondeo 4 cifras signif y variable categorica
...
#lista de pozos. Variable categorica
...
#valores RO:
valores_ro <- as.data.frame(sqldf("SELECT WELL, MAX(RO), MIN(RO), ROUND(AVG(RO),2) \"AVG(RO)\" FROM datos_POZOS_DET GROUP BY WELL"))
valores_ro$ro_originales <- c(1.3, 1.6, 1.55, 1.45, 1.55, 1.6, 0.9, 1, 1, 1.1, 1.22, 1.08, 1.2, 1.78)
